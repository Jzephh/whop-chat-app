...
export default function Home() {
  ...
  async function sendMessageWith(pastedFile?: File | null) {
    ...
    const fileToUpload = pastedFile || imageFile;
    if (fileToUpload) {
      ...
      if (!pastedFile) setImageFile(null);
    ...
    <div className="min-h-screen w-full flex flex-col">
      ...
      <div className="sticky bottom-0 w-full">
        <div className="relative bg-[#151517] border-t border-[var(--border)] flex items-center">
            ...
            onPaste={(e) => {
              ...
              for (let i = 0; i < dt.items.length; i++) {
                ...
                if (it.kind === 'file' && it.type.startsWith('image/')) {
                  ...
                  if (f) {
                    e.preventDefault();
                    // Send immediately with pasted image
                    void sendMessageWith(f);
                    return;
                  }
                }
              }
          ...
          />
          <input type="file" accept="image/*" onChange={e => { const f = e.target.files?.[0] || null; if (f) { void sendMessageWith(f); } else { setImageFile(null); } }} className="hidden" id="fileInput" />
